#include "./process_h/chars_identify.h"
...
int CCharsIdentify::classify(Mat f, bool isChinses,bool isSpeci,double& rate){
    ...
    if (!isChinses)//对中文字符的判断
    {
        ...
        {
            result = 0;
            float maxVal = -2,secondVal=-3,thirdVal = -4;
            ...
            rate = maxVal/(maxVal+secondVal+thirdVal);
...
string CCharsIdentify::charsIdentify(Mat input, bool isChinese,bool isSpeci,double& rate)
{
    Mat f = features(input, m_predictSize);
    string result = "";
    int index = classify(f, isChinese,isSpeci,rate);//使用ann来判别那个字符
